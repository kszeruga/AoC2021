...
namespace Day19// Note: actual namespace depends on the project name.
{
    ...
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            scanners.RemoveAt(0);
            var zeros = new List<Point3D> {new Point3D(0, 0, 0) };
            while (scanners.Count > 0)
            {
                ...
                foreach (var scan in matched)
                {
                    ...
                    {
                        var (ok, add,zero) = IsOverlap(scan, a);
                        if (ok)
                        {
                            zeros.Add(zero);
            ...
            System.Console.WriteLine(zeros.Join(zeros, point3D => true, point3D => true, (x, y) => x.Distance(y)).Max());

        }

        private static (bool, IEnumerable<Point3D> msr2, Point3D) IsOverlap(List<Point3D> s1, List<Point3D> s2)
        {
            foreach (var p1 in s1)
            {
                foreach (var p2 in s2)
                {
                    ...
                    foreach (var rot in Rotations)
                    {
                        ...
                        {
                            var z = new Point3D(0, 0, 0);
                            z = z.Move(trans_vaector).Move(rev_trans_vaector).Rotate(rot).Move(p1.matrix);
                            return (true, msr2, z);
                        }
    ...
    public class Point3D : IEquatable<Point3D>
    {
        ...
        public long Distance(Point3D a)
        {
